...
def detect_aruco_tags(
    ...
    for this_id, this_corners in zip(ids, corners):
        ...
        # Estimate pose using solvePnP (estimatePoseSingleMarkers removed in OpenCV 4.8+)
        ...
        corners_2d = this_corners.reshape(4, 2).astype(np.float64)
        success, rvec, tvec = cv2.solvePnP(
            obj_pts, corners_2d, K, zero_dist, flags=cv2.SOLVEPNP_IPPE_SQUARE
        )
        if not success:
            continue
